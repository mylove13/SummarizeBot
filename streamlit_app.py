...
def load_user_data():
    try:
        ...
        return {}  # 파일이 비어 있거나 JSON 형식이 아닌 경우 빈 딕셔너리 반환
    except FileNotFoundError:
        # 파일이 없는 경우에도 빈 딕셔너리를 반환하여 오류를 방지하고, 이후에 생성될 수 있도록 함
        return {}
...
def save_user_data(data):
    try:
        with open(USER_DATA_FILE, "w", encoding="utf-8") as f:
            json.dump(data, f, ensure_ascii=False, indent=4)  # ensure_ascii=False, indent=4 추가
...
mode = st.sidebar.selectbox("선택", ["로그인", "회원가입"])
...
if mode == "회원가입":
    ...
    if st.sidebar.button("회원가입"):
        if username in user_data:
            st.sidebar.error("❌ 이미 존재하는 사용자명입니다.")
        else:
            user_data[username] = {"password": hash_password(password), "user_id": str(uuid.uuid4())}
            save_user_data(user_data) # 여기서 저장 함수 호출
            st.sidebar.success("✅ 회원가입 성공! 로그인하세요.")
...
if mode == "로그인":
    ...
    if st.sidebar.button("로그인"):
        if username in user_data and user_data[username]["password"] == hash_password(password):
            ...
            st.session_state.username = username  # 사용자 이름을 세션에 저장
            st.sidebar.success(f"✅ 로그인 성공! 환영합니다, {username}!")
        else:
            st.sidebar.error("❌ 사용자명 또는 비밀번호가 잘못되었습니다.")

# ✅ 로그아웃 기능
if "user_id" in st.session_state:
    if st.sidebar.button("로그아웃"):
        del st.session_state.user_id
        del st.session_state.username # 세션에서 사용자 이름 삭제
...
username = st.session_state.get("username", "Unknown User") # 세션에서 사용자 이름을 가져오거나 기본값 설정

st.sidebar.info(f"현재 로그인된 사용자 ID: {user_id} ({username})") # username 표시
...
def load_articles(filename="news_articles.json"):
    if os.path.exists(filename):
        try:
            ...
            return []
        except Exception as e:  # 추가: 다른 예외 처리
